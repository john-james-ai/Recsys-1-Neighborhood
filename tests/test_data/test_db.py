...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.6                                                                              #
# Filename   : /tests/test_data/test_db.py                                                         #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
class TestDB:  # pragma: no cover
    ...
    # ============================================================================================ #
    def test_create(self, container, dtos, caplog):
        start = datetime.now()
        logger.info(
            "\n\nStarted {} {} at {} on {}".format(
                self.__class__.__name__,
    ...
    def test_read(self, container, caplog):
        ...
        for i in range(1, 6):
            ...
            assert dto.cols == i + 14
            assert dto.n_users == i * 1000
            assert dto.n_items == i * 2000
            assert dto.size == i + 8772
            assert dto.matrix_size == i + 5362
            assert dto.memory_mb == i * 46
            assert dto.cost == i * 264
            assert dto.sparsity == 85
            assert dto.density == 15
    ...
    # ============================================================================================ #
    def test_update(self, container, dtos, caplog):
        start = datetime.now()
        logger.info(
            "\n\nStarted {} {} at {} on {}".format(
                self.__class__.__name__,
        ...
        for i in range(1, 6):
            ...
            dto.rows = i + 55
            dto.cols = i + 88
            dto.matrix_size = i * 55
            dao.update(dto)

        for i in range(1, 6):
            dto = dao.read(i)
            logger.debug(dto)
            assert dto.rows == i + 55
            assert dto.cols == i + 88
            assert dto.matrix_size == i * 55
